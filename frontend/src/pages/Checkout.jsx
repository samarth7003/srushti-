      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="max-w-md mx-auto mb-12">
          <div className="flex items-center justify-between relative">
            {/* Background Line */}
            <div className="absolute left-0 right-0 top-1/2 -translate-y-1/2 h-[2px] bg-stone-200 dark:bg-stone-800 z-0"></div>
            {/* Active Progress Line */}
            <div 
              className="absolute left-0 top-1/2 -translate-y-1/2 h-[2px] bg-gold-500 transition-all duration-500 z-0"
            ...
            <button
              type="button"
              onClick={() => setStep(1)}
              className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 border-2 cursor-pointer ${
                step === 1
                  ? "bg-gold-500 text-stone-950 border-gold-500 shadow-md shadow-gold-500/20"
                  : "bg-white dark:bg-luxury-charcoal text-gold-500 border-gold-500"
              }`}
            >
              1
            </button>
            ...
            <button
              type="button"
              disabled={!name || !mobile || !address || !city || !pincode || mobile.length < 10 || pincode.length !== 6}
              onClick={() => {
                if (validateShippingDetails()) setStep(2);
              }}
              className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 border-2 ${
                step === 2
                  ? "bg-gold-500 text-stone-950 border-gold-500 shadow-md shadow-gold-500/20"
                  : "bg-white dark:bg-luxury-charcoal text-gray-400 border-stone-200 dark:border-stone-850 disabled:cursor-not-allowed cursor-pointer disabled:opacity-50"
              }`}
            >
              2
            </button>
          </div>
          <div className="flex justify-between mt-3 text-[10px] font-semibold uppercase tracking-wider">
            <span className={step === 1 ? "text-gold-600 dark:text-gold-400" : "text-gray-400"}>Shipping Address</span>
            <span className={step === 2 ? "text-gold-600 dark:text-gold-400" : "text-gray-400"}>Payment Details</span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-8 space-y-8 text-left">
            {step === 1 ? (
              ...
              <div className="bg-white dark:bg-luxury-charcoal p-6 rounded-2xl border border-gold-100/50 dark:border-stone-850/40 shadow-sm space-y-4 animate-fade-in">
                <h3 className="font-serif text-xl font-bold text-luxury-black dark:text-white border-b border-gold-100/15 dark:border-stone-800/20 pb-3 flex items-center gap-2">
                  <span className="text-gold-500">1.</span> Delivery & Contact Details
                </h3>
                ...
                <div className="pt-4 flex justify-end">
                  <button
                    ...
                    className="bg-gold-500 hover:bg-gold-600 text-stone-950 font-bold uppercase tracking-wider py-3.5 px-8 rounded-xl text-xs transition-all shadow-md cursor-pointer flex items-center gap-2"
                  >
              ...
              /* Step 2: Payment details options */
              <div className="space-y-6 animate-slide-up">
                {/* Compact shipping details preview */}
                <div className="bg-stone-50 dark:bg-stone-900/60 p-5 rounded-2xl border border-gold-200/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="space-y-1">
                    <h4 className="font-serif text-sm font-bold text-luxury-black dark:text-white flex items-center gap-2">
                      📍 Shipping to: <span className="text-gold-600 dark:text-gold-400 font-sans font-semibold text-xs">{name} ({mobile})</span>
                    </h4>
                  ...
                  <button
                    type="button"
                    onClick={() => setStep(1)}
                    className="border border-gold-400/40 text-gold-600 dark:text-gold-400 hover:bg-gold-500/10 px-4 py-2 rounded-xl text-[11px] font-bold uppercase tracking-wider transition-all self-start md:self-auto cursor-pointer"
                  >
                    Edit Info
                  </button>
                </div>
                ...
                <div className="bg-white dark:bg-luxury-charcoal p-6 rounded-2xl border border-gold-100/50 dark:border-stone-850/40 shadow-sm space-y-6">
                  <h3 className="font-serif text-xl font-bold text-luxury-black dark:text-white border-b border-gold-100/15 dark:border-stone-800/20 pb-3 flex items-center gap-2">
                    <span className="text-gold-500">2.</span> Select Luxury Payment Method
                  </h3>
                  ...
                  <div className="pt-4 flex justify-between items-center border-t border-gold-100/15 dark:border-stone-800/20">
                    <button
                      type="button"
                      onClick={() => setStep(1)}
                      className="text-xs font-semibold text-gold-600 dark:text-gold-400 hover:text-gold-500 uppercase tracking-widest cursor-pointer"
                    >
                    ...
                    <button
                      type="submit"
                      disabled={submitting}
                      className="bg-gold-500 hover:bg-gold-600 disabled:bg-gold-200/50 text-stone-950 font-bold uppercase tracking-wider py-3.5 px-8 rounded-xl text-xs transition-all shadow-md cursor-pointer flex items-center justify-center gap-2"
                    >
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white dark:bg-luxury-charcoal p-6 rounded-3xl border border-gold-100/50 dark:border-stone-850/40 shadow-sm space-y-6">
              ...
              <button
                ...
                onClick={step === 1 ? handleProceedToPayment : undefined}
                disabled={submitting}
